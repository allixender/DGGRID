...
#define ZORDER_GET_QUADNUM(z)                                                  \
  ((int)((((z) & ZORDER_QUAD_MASK) >> ZORDER_QUAD_OFFSET)))

/** Sets the integer mode of z to v. */
#define ZORDER_SET_QUADNUM(z, v)                                               \
  (z) = (((z) & ZORDER_QUAD_MASK_NEGATIVE) |                                   \
         (((uint64_t)(v)) << ZORDER_QUAD_OFFSET))
...
#define ZORDER_SET_INDEX_DIGIT(z, res, digit)                                  \
  (z) = (((z) & ~((ZORDER_DIGIT_MASK << ((MAX_ZORDER_RES - (res)) *            \
...
////////////////////////////////////////////////////////////////////////////////
const char *DgZOrderRF::str2add(DgZOrderCoord *add, const char *str,
...
////////////////////////////////////////////////////////////////////////////////
string DgZOrderCoord::valString(void) const {
...
DgZOrderStringtoZOrderConverter::DgZOrderStringtoZOrderConverter(
    ...
    : DgConverter<DgZOrderStringCoord, long long int, DgZOrderCoord,
                  long long int>(from, to) {
  // validate the to/from RF's
  const DgZOrderStringRF *zsRF =
      dynamic_cast<const DgZOrderStringRF *>(&fromFrame());
  if (!zsRF) {
    report(
        "DgZOrderStringtoZOrderConverter::DgZOrderStringtoZOrderConverter(): "
        " fromFrame not of type DgZOrderStringRF",
        DgBase::Fatal);
  }
  ...
  if (!zRF) {
    report(
        "DgZOrderStringtoZOrderConverter::DgZOrderStringtoZOrderConverter(): "
        " toFrame not of type DgZOrderRF",
        DgBase::Fatal);
  }
  ...
  if (zsRF->aperture() != zRF->aperture() || zsRF->res() != zRF->res()) {
    report(
        "DgZOrderStringtoZOrderConverter::DgZOrderStringtoZOrderConverter(): "
        " fromFrame and toFrame apertures or resolutions do not match",
        DgBase::Fatal);
  }
  ...
  for (const char &digit : radStr) {
    if (r > res_)
      report("DgZOrderStringtoZOrderConverter::convertTypedAddress(): "
             " incoming index exceeds converter resolution",
             DgBase::Fatal);
...
DgZOrderToZOrderStringConverter::DgZOrderToZOrderStringConverter(
    ...
    : DgConverter<DgZOrderCoord, long long int, DgZOrderStringCoord,
                  long long int>(from, to) {
  // validate the to/from RF's
  ...
  if (!zRF) {
    report(
        "DgZOrderToZOrderStringConverter::DgZOrderToZOrderStringConverter(): "
        " fromFrame not of type DgZOrderRF",
        DgBase::Fatal);
  }

  const DgZOrderStringRF *zsRF =
      dynamic_cast<const DgZOrderStringRF *>(&toFrame());
  if (!zsRF) {
    report(
        "DgZOrderToZOrderStringConverter::DgZOrderToZOrderStringConverter(): "
        " toFrame not of type DgZOrderStringRF",
        DgBase::Fatal);
  }
